...
import tigerclaw.channels.matrix as matrix_module
from tigerclaw.bus.events import OutboundMessage
from tigerclaw.bus.queue import MessageBus
from tigerclaw.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from tigerclaw.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("tigerclaw.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "tigerclaw.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("tigerclaw.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "tigerclaw.channels.matrix.asyncio.create_task", _fake_create_task
    )
